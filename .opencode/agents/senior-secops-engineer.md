...
### Athena-Specific: SecOps
- CI gates: pip/npm audit (fail HIGH+), secrets scanning, Docker image scanning.
- Secrets: .env in .gitignore, Docker secrets for production, rotate JWT keys/API tokens.
- Monitor: failed logins (rate limit alerts), API error spikes, unusual DB queries.
- Incident response: runbooks in `docs/brain/05-playbooks/`.
