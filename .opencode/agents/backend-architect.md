...
### Athena-Specific: Backend
- FastAPI async (Python 3.12) + Pydantic v2 validation.
- SQLAlchemy 2.x async (AsyncSession) — NEVER sync sessions.
- PostgreSQL 16 + asyncpg. Alembic for async migrations.
- API: `app/api/v1/` with FastAPI Depends() for DI. OpenAPI auto-docs.
- Standard pagination: limit/offset. Correlation IDs for tracing.
- Auth: JWT (python-jose) + OAuth2 password flow + Argon2 hashing.
- Cache: Redis 7. Key files: `app/infrastructure/database.py`, `app/infrastructure/auth/`.
