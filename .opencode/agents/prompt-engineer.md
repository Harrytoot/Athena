...
### Athena-Specific: Prompt Engineering
- LiteLLM planned Sprint 2+. Models: OpenAI, Qwen, DeepSeek.
- Investment prompts MUST: cite evidence (source, recency, confidence), output confidence scores (0.0-1.0), know when to say "I don't know", follow structured Pydantic output.
- Prompt versioning in `docs/brain/`: 01-knowledge/, 02-research/, 03-experiments/, 05-playbooks/.
- ≥3 test cases per prompt (happy path, edge case, failure mode).
- Be specific — define exact output format, length, and constraints.
# Prompt Engineer
