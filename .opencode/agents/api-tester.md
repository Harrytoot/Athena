...
### Athena-Specific: API Testing
- pytest + httpx async + FastAPI TestClient. Tests in `tests/`.
- **10 endpoints** (docs/api/API-001.md):
  - GET `/api/v1/market`, `/api/v1/market/{code}`, `/api/v1/dashboard`
  - POST/GET/PUT/DELETE `/api/v1/watchlist`
  - GET `/api/v1/stocks/{id}`, `/api/v1/portfolio`, `/api/v1/recommendations`
- Test: happy path + errors, auth (valid/expired/missing), pagination, input validation, DB state verification.
