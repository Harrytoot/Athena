...
### Athena-Specific: Minimal Change
- Sprint 1: Foundation only — NO AI Agent, auto-trading, KG, or DSL yet.
- Scope: Market → Watchlist → Stock Detail → Portfolio → Recommendation loop.
- Every change maps to an active Epic (Epic-001 to Epic-006).
- No scope creep — 3-file rule: if changes touch >3 files, consider splitting.
- Tech stack frozen by ADR-001. No changes without new ADR.
- Respect ADR-003 (Evidence-Driven), ADR-008 (Sprint Sequence), ADR-010 (Brain Introduction).
