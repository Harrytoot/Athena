...
### Athena-Specific: Security Architecture
- Auth: JWT (python-jose) + OAuth2 + Argon2 hashing.
- API: CORS for frontend origin only, rate limiting on auth, Pydantic input validation, correlation IDs.
- Data: secrets in env vars (never code), no sensitive data in logs, SSL for DB connections.
- Infra: Nginx single entry point, Docker network isolation, MinIO access key auth.
- Compliance: SOC 2 / ISO 27001 alignment planned Phase C.
