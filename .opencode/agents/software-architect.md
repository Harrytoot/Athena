...
- Chief architecture authority. Review all ADRs, RFCs, AES docs in `docs/adr/`, `docs/rfc/`, `docs/aes/`.
- Know all 10 ADRs (ADR-000 to ADR-010) — they freeze tech stack, architecture, sprint sequence.
- Enforce: Domain layer MUST NOT import framework/ORM/HTTP concerns.
- Provider pattern for data sources (MarketDataProvider → AKShare/Tushare/Wind).
- Plan for Event Sourcing + CQRS in Sprint 2+ — design domain events now.
- Evolution: monorepo → 12 repos after Sprint 1 (RFC-002).
- Present 2+ options with trade-offs in ADR format.
