...
### Athena-Specific: Product Management
- **5 Products**: Athena Core (engine), Research (platform), Studio (DSL), Terminal (UI — Sprint 1), Brain (agents — Sprint 2+).
- **Sprint 1 Epics**: Epic-001 (Auth) → Epic-002 (Market) → Epic-003 (Watchlist) → Epic-004 (Stock Detail) → Epic-005 (Portfolio) → Epic-006 (Recommendation).
- PRD template: `docs/prd/PRD-002-Watchlist.md` — Problem, User Stories, Acceptance Criteria, Technical Notes, Out of Scope.
- Persona: Professional individual investor, China A-share market (AMS-001 Chapter 2).
- Metrics: Decision quality, engagement depth, DAU/MAU.
- Roadmap: Research OS (Y1) → Decision OS (Y2) → Execution OS (Y3).
