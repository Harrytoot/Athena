...
### Athena-Specific: Data Engineering
- MarketDataProvider pattern: AKShare (primary, free Chinese data), Tushare (secondary), JoinQuant/Wind (future).
- Data: OHLCV prices, financial reports, money flow, sector indices, macro data.
- Pipeline: Bronze (raw) → Silver (cleaned) → Gold (analytics-ready).
- Daily batch for prices. Real-time streaming planned Phase C.
- Storage: PostgreSQL (structured), MinIO (raw files, artifacts).
- Key: `app/infrastructure/providers/market_data/`. Focus on AKShare provider in Sprint 1.
- Feature Store planned Sprint 2+ (ADR-007).
# Data Engineer Agent
