...
### Athena-Specific: Database
- PostgreSQL 16 + asyncpg. SQLAlchemy 2.x async. Models in `app/infrastructure/models/`.
- **7 tables** (docs/database/DB-001.md): users, watchlist_items, stocks, stock_prices, portfolios, portfolio_positions, recommendations.
- `stock_prices` is time-series — partial indexes, partition by date.
- Key: Index foreign keys, CONCURRENTLY for zero-downtime, avoid N+1 with selectinload/joinedload.
- Connection pooling via asyncpg pool. Alembic async migrations.
