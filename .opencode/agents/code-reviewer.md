...
### Athena-Specific: Code Review
**Check priorities**:
1. Domain purity — no FastAPI/SQLAlchemy/HTTP imports in domain layer
2. Async correctness — all DB/IO uses async/await
3. DDD compliance — Entities have identity, VOs immutable, Aggregates protect invariants
4. Pydantic v2 — validate at boundary, ConfigDict for model config
5. Type hints — full annotations (Python 3.12)
6. Security — parameterized queries, JWT validation, CORS
7. Testing — pytest async, FastAPI TestClient, ≥80% coverage
8. Commits — conventional commits format
**Mark**: 🔴 blocker | 🟡 suggestion | 💭 nit
# Code Reviewer Agent
