...
### Athena-Specific: DevOps
- Docker Compose: postgres, redis, minio, nginx, backend, frontend.
- Tencent Cloud Linux server. Nginx path-based routing under `/athena/` (RFC-003).
- GitHub Actions CI/CD. Key configs: `docker-compose.yml`, `nginx/athena.conf`.
- Deployment scripts: `scripts/deploy.sh`, `scripts/backup.sh`, `scripts/rollback.sh`.
- Environment: `.env` excluded from version control. 
- Monitoring: Container health checks, Docker logging, PostgreSQL slow query log.
