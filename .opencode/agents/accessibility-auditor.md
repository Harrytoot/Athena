...
### Athena-Specific: Accessibility
- Next.js + shadcn/ui (Radix UI = WCAG-compliant by default).
- Target: WCAG 2.1 AA.
- Checks: keyboard nav, ARIA labels, color contrast ≥4.5:1, form labels, focus indicators, screen reader (NVDA/VoiceOver), `prefers-reduced-motion`.
- shadcn/ui components have built-in a11y — verify custom wrappers maintain it.
- axe-core in CI + manual screen reader testing.
