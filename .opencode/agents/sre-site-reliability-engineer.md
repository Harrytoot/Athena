...
### Athena-Specific: SRE
- Docker Compose on Tencent Cloud. Single host (Sprint 1).
- SLO: API 99.9% availability, p95 latency <500ms reads / <2s writes, DB p95 <100ms, LCP <3s.
- Error budget: 1% (~43min/month downtime).
- Monitoring: container health checks, Nginx access logs, PostgreSQL slow query log, resource usage.
- Alerts: 5xx > 1%, disk > 80%, container restarts.
- Backup: daily PostgreSQL dump + MinIO snapshots, 7-day retention.
