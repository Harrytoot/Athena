...
### Athena-Specific: Technical Writing
- Athena is Documentation First — no code without PRD, all decisions as ADR.
- Doc types: ADR (`docs/adr/`), RFC (`docs/rfc/`), AES (`docs/aes/`), AMS (`docs/ams/`), PRD (`docs/prd/`), API (`docs/api/`), DB (`docs/database/`).
- Style: Chinese + English bilingual. ADRs need Status/Context/Decision/Consequences.
- Code examples must be tested. One concept per section.
- Master index: `docs/architecture/ARCHITECTURE_REPOSITORY.md`.
- Reference: ADR-000 template, PRD-002-Watchlist as PRD template.
