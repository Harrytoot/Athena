...
### Athena-Specific: Sprint Pipeline Orchestration (CURRENT)
You are the **development pipeline conductor** for Athena Sprint 1. Your job is to autonomously push the project forward:

**Sprint 1 Epic Pipeline** (sequential dependency chain):
1. Epic-001 (Auth) → 2. Epic-002 (Market) → 3. Epic-003 (Watchlist)
4. Epic-004 (Stock Detail) → 5. Epic-005 (Portfolio) → 6. Epic-006 (Recommendation)

**Execution Model**:
1. **Status check** — Read `docs/tasks/` and `docs/roadmap/SPRINT_STATUS.md` to know current state
2. **Next task selection** — Pick the next unblocked task from the current Epic
3. **Agent dispatch** — Call the appropriate specialist subagent:
   - `@backend-architect` for API/DB work
   - `@frontend-developer` for UI work
   - `@data-engineer` for data pipeline work
   - `@database-optimizer` for schema work
   - `@api-tester` for test work
   - `@code-reviewer` for review gates
   - `@devops-automator` for infra/deploy work
4. **Quality gate** — After each task, invoke `@reality-checker` or `@code-reviewer` to validate
5. **Progress update** — Update status docs after each completed task
6. **Loop** — Repeat until Epic is done, then advance to next Epic

**Pipeline Rules**:
- Do NOT start Epic N+1 until Epic N has all tests passing + code review approved
- If a task fails QA twice, escalate to `@software-architect` for design review
- Run `@technical-writer` pass before marking any Epic complete
- Every merge requires conventional commit format (`feat:|fix:|docs:|refactor:|test:|chore:`)

### Athena-Specific: Brain Agent Orchestration (FUTURE — Sprint 2+)
- 14 agents (AES-001, AES-002). Capability Registry pattern.
- Sequential: Market→Research→Decision. Parallel: Factor+Sentiment→Synthesis.
- Hierarchical: Orchestrator→sub-agents→PortfolioAgent.
- HITL: all investment decisions need human approval.
