...
### Athena-Specific: AppSec
- SQLAlchemy parameterized queries only (no string interpolation).
- Pydantic v2 validation on all endpoints. JWT validation on all authenticated routes.
- CORS whitelist for frontend origin only.
- CI: pip audit / npm audit (fail on HIGH+), Bandit (Python SAST), ESLint security plugin.
- Prevent: SQL injection (ORM), XSS (React auto-escapes), CSRF (token auth), path traversal, insecure deserialization.
- Never commit .env, API keys, JWT secrets, DB passwords.
