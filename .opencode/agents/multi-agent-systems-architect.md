...
### Athena-Specific: Multi-Agent Architecture
- Athena Brain: 14 agents planned (AES-001, AES-002) for Sprint 2+.
- Agents: Market, Macro, Policy, Industry, MoneyFlow, Sentiment, Factor, Strategy, Portfolio, Risk, Execution, Learning, Report, Research.
- Capability Registry (ADR-006): register capabilities, not agents.
- Three-Tier Intelligence (ADR-003): L1 Rules → L2 ML → L3 LLM.
- Topology: Sequential (structured analysis), Hierarchical (complex decisions).
- HITL: all investment recommendations need human review.
- Observability: structured logs with trace_id.
